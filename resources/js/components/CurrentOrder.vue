    ...
    <div class="order-list-container">
        ...
        <div class="filters">
            <select v-model="statusFilter" class="form-control">
                <option :value="CONSTANTS.PENDING_PROCESSING">All Statuses</option>
                <option :value="CONSTANTS.ORDER_STATUS_PENDING">Pending</option>
                <option :value="CONSTANTS.ORDER_STATUS_PROCESSING">Processing</option>
            </select>
        </div>
                ...
                <tbody>
                    <tr v-for="order in orders" :key="order.id">
                        ...
                        <td>
                            ...
                            <router-link :to="{ name: 'user-order-payment', params: { orderId: order.id } }" class="btn-view" style="margin-right: 0.5rem;">
                                Payment
                            </router-link>
...
const error = ref('');
const statusFilter = ref(route.query.status || CONSTANTS.PENDING_PROCESSING);

const emptyStateMessage = computed(() => {
    if (statusFilter.value === CONSTANTS.ORDER_STATUS_PENDING) {
        return 'No Pending orders found.';
    } else if (statusFilter.value === CONSTANTS.ORDER_STATUS_PROCESSING) {
        return 'No Processing orders found.';
    }
...
watch(() => route.query.status, (newStatus) => {
    statusFilter.value = newStatus || CONSTANTS.PENDING_PROCESSING;
