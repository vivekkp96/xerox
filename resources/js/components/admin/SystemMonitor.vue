...
watch(memoryStats, (current, previous) => {
    if (previous && current.used_memory_bytes !== previous.used_memory_bytes) {
        ...
        setTimeout(() => statsChanged.value = false, 500);
    }
});
...
watch(diskStats, (current, previous) => {
    if (previous && current.occupied_space_bytes !== previous.occupied_space_bytes) {
        ...
        setTimeout(() => diskStatsChanged.value = false, 500);
    }
});
...
watch(cpuLoadStats, (current, previous) => {
    if (previous) {
        ...
        if (changed) {
            ...
            setTimeout(() => cpuStatsChanged.value = false, 500);
        }
    }
});
...
watch(mysqlStats, (current, previous) => {
    if (previous && (current.cpu_usage_percent !== previous.cpu_usage_percent || current.memory_usage_bytes !== previous.memory_usage_bytes)) {
        ...
        setTimeout(() => mysqlStatsChanged.value = false, 500);
    }
});
...
const getDiskBarColor = (percent: number) => {
    if (percent <= 60) return CONSTANTS.COLOR?.GREEN ?? '#28a745'; // green
    if (percent <= 85) return CONSTANTS.COLOR?.ORANGE ?? '#ffc107'; // orange
    return CONSTANTS.COLOR?.RED ?? '#dc3545'; // red
};
...
const fetchMemoryStats = async () => {
    try {
        ...
        const response = await axios.get(CONSTANTS.API.ADMIN_SYSTEM_MEMORY, {
            headers: {
...
const fetchDiskStats = async () => {
    try {
        ...
        const response = await axios.get(CONSTANTS.API.ADMIN_DISK_SPACE, {
            headers: {
                'Authorization': `Bearer ${token}`,
                'Accept': 'application/json'
            }
...
const fetchCpuLoadStats = async () => {
    try {
        ...
        const response = await axios.get(CONSTANTS.API.ADMIN_CPU_LOAD, {
            headers: {
                'Authorization': `Bearer ${token}`,
                'Accept': 'application/json'
            }
...
const fetchMysqlStats = async () => {
    try {
        ...
        const response = await axios.get(CONSTANTS.API.ADMIN_MYSQL_USAGE, {
            headers: {
                'Authorization': `Bearer ${token}`,
                'Accept': 'application/json'
            }
...
onMounted(() => {
    ...
    intervalId = setInterval(() => {
        fetchMemoryStats();
    }, CONSTANTS.FETCH_MEMORY_STATS_API_DELAY_30 || 30000);
    intervalId1 = setInterval(() => {
        fetchDiskStats();
    }, CONSTANTS.FETCH_MEMORY_STATS_API_DELAY_70 || 70000);
    intervalId2 = setInterval(() => {
        fetchCpuLoadStats();
    }, CONSTANTS.FETCH_MEMORY_STATS_API_DELAY_40 || 40000);
...
.card-title { font-size: 0.75rem; font-weight: 700; color: #adb5bd; letter-spacing: 0.5px; text-transform: uppercase; }
.card-icon { color: #e9ecef; background-color: #f8f9fa; padding: 8px; border-radius: 8px; display: flex; align-items: center; justify-content: center; }
.card-icon svg { color: #007bff; }
.main-value { font-size: 2.5rem; font-weight: 700; color: #2c3e50; line-height: 1; margin-bottom: 0.5rem; }
.unit { font-size: 1.25rem; font-weight: 500; color: #6c757d; margin-left: 2px; }
.progress-container { height: 6px; background-color: #e9ecef; border-radius: 3px; margin: 1rem 0; overflow: hidden; }
.progress-bar { height: 100%; border-radius: 3px; transition: width 0.5s ease; }
.details-list { margin-top: 1rem; }
.detail-item { display: flex; justify-content: space-between; margin-bottom: 0.5rem; font-size: 0.9rem; }
.label { color: #6c757d; }
.value { font-weight: 600; color: #495057; transition: color 0.3s ease; }
.value-change-animation { color: #007bff; }
.loading-state { color: #adb5bd; font-style: italic; }
</style>
